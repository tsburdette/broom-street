...
class PutAction(Action):
    def __init__(self, args):
        match = re.match(r'(?P<item>.*)(?: IN )(?(1)(?P<destination>.*))', args) or re.match(r'(?P<item>.+)', args)
        super().__init__(method=Game.put_item, target=match.group('item'),
                destination=match.group('destination') if len(match.groups()) > 1 else None)
